...
const Navbar = ({pageTitle, onHandleSidebar}: NavbarProps) => {
	...
	return (
      ...
      <div className="flex items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit">
          ...
          <ol className="flex flex-wrap pt-1 mr-12 bg-transparent rounded-lg sm:mr-16">
            <li className="leading-normal text-sm breadcrumb-item">
              <a className="text-slate-700 opacity-30" href="#">
                <ShopIcon className="w-[12px] h-[12px] mb-1" />
              </a>
            </li>
            ...
            <li className="text-sm pl-2 capitalize leading-normal text-slate-700 before:float-left before:pr-2 before:text-gray-600 before:content-['/']" aria-current="page">{pageTitle}</li>
          </ol>
          <h6 className="mb-0 font-bold capitalize">{pageTitle}</h6>
        </nav>

        <div className="flex items-center">
          <button onClick={onHandleSidebar} className="hidden p-0 transition-all ease-nav-brand text-sm text-slate-500 xl:block">
        ...
        <div className="flex items-center mt-2 grow sm:mt-0 sm:mr-6 md:mr-0 lg:flex lg:basis-auto" id="navbar">
          ...
          <ul className="flex flex-row justify-end pl-0 items-center mb-0 list-none md-max:w-full">
            <li className="flex h-12 items-center p-0">
              <Link to="/dashboard/profile" className="block p-0 transition-all ease-nav-brand text-sm text-slate-500 flex items-center font-semibold">
                <UserIcon className="md:mr-1 w-5 h-5" />
                <span className="hidden sm:inline">{user?.name}</span>
              </Link>
            </li>

            <li className="flex h-12 items-center pl-3 md:pl-4 xl:hidden">
              <a sidenav-trigger="" className="block p-0 transition-all ease-nav-brand text-sm text-slate-500" href="#" aria-expanded="false">
            ...
            <li className="relative h-12 flex items-center pr-2">
              <p className="hidden transform-dropdown-show"></p>
              <a dropdown-trigger="" href="#" className="block p-0 transition-all text-sm ease-nav-brand text-slate-500" aria-expanded="false">
              ...
              <ul dropdown-menu="" className="text-sm transform-dropdown before:font-awesome before:leading-default before:duration-350 before:ease-soft lg:shadow-soft-3xl duration-250 min-w-44 before:sm:right-7 before:text-5.5 pointer-events-none absolute right-0 top-0 z-50 origin-top list-none rounded-lg border-0 border-solid border-transparent bg-white bg-clip-padding px-2 py-4 text-left text-slate-500 opacity-0 transition-all before:absolute before:right-2 before:left-auto before:top-0 before:z-50 before:inline-block before:font-normal before:text-white before:antialiased before:transition-all before:content-['\f0d8'] sm:-mr-6 lg:absolute lg:right-0 lg:left-auto lg:mt-2 lg:block lg:cursor-pointer">
                ...
                <li className="relative mb-2">
                  <a className="group ease-soft py-1.2 clear-both block w-full whitespace-nowrap rounded-lg bg-transparent px-4 duration-300 hover:bg-gray-200 hover:text-slate-700 lg:transition-colors" href="#">
                    <div className="flex py-1">
                      ...
                      <div className="flex flex-col justify-center">
                        <h6 className="mb-1 font-normal leading-normal text-sm group-hover:text-slate-700"><span className="font-semibold">New message</span> from Laur</h6>
                        <p className="mb-0 leading-tight text-xs text-slate-400 group-hover:text-slate-700">
